...
func Serve(terminal_port string, code_port string, agent_port string, vnc_port string, vnc2_port string) {
	...
	vnc := VNCReverseProxy("http://localhost:" + vnc_port)
	vnc2 := VNC2ReverseProxy("http://localhost:" + vnc2_port)
	http.Handle("/", codee)
	http.Handle("/term/", terminal)
	http.Handle("/vnc/", vnc)
	http.Handle("/vnc2/", vnc2)
...
func VNCReverseProxy(target string) *httputil.ReverseProxy {
	url, _ := url.Parse(target)
	return &httputil.ReverseProxy{
		Director: func(req *http.Request) {
			req.URL.Scheme = url.Scheme
			req.URL.Path = strings.Replace(req.URL.Path, "/vnc/", "/", 1)
			req.URL.Host = url.Host
		},
	}
}

func VNC2ReverseProxy(target string) *httputil.ReverseProxy {
	url, _ := url.Parse(target)
	return &httputil.ReverseProxy{
		Director: func(req *http.Request) {
			req.URL.Scheme = url.Scheme
			req.URL.Path = strings.Replace(req.URL.Path, "/vnc2/", "/", 1)
			req.URL.Host = url.Host
		},
	}
}
